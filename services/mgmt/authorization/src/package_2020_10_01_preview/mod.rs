...
pub mod role_assignments {
    ...
    pub mod validate_by_id {
        ...
        impl RequestBuilder {
            ...
            fn url(&self) -> azure_core::Result<azure_core::Url> {
                let mut url = azure_core::Url::parse(&format!("{}/{}/validate", self.client.endpoint(), &self.role_assignment_id))?;
                let has_api_version_already = url.query_pairs().any(|(k, _)| k == azure_core::query_param::API_VERSION);
                if !has_api_version_already {
                    url.query_pairs_mut()
                        .append_pair(azure_core::query_param::API_VERSION, "2020-10-01-preview");
...
pub mod role_assignment_metrics {
    ...
    impl Client {
        ...
        #[doc = "Arguments:"]
        #[doc = "* `subscription_id`: The ID of the target subscription."]
        pub fn get_metrics_for_subscription(&self, subscription_id: impl Into<String>) -> get_metrics_for_subscription::RequestBuilder {
            get_metrics_for_subscription::RequestBuilder {
                client: self.0.clone(),
                subscription_id: subscription_id.into(),
            }
        }
    ...
    pub mod get_metrics_for_subscription {
        ...
        pub struct Response(azure_core::Response);
        impl Response {
            pub async fn into_body(self) -> azure_core::Result<models::RoleAssignmentMetricsResult> {
                let bytes = self.0.into_body().collect().await?;
                let body: models::RoleAssignmentMetricsResult = serde_json::from_slice(&bytes)?;
                Ok(body)
            }
            pub fn into_raw_response(self) -> azure_core::Response {
                self.0
        ...
        impl std::future::IntoFuture for RequestBuilder {
            type Output = azure_core::Result<models::RoleAssignmentMetricsResult>;
            type IntoFuture = BoxFuture<'static, azure_core::Result<models::RoleAssignmentMetricsResult>>;
            #[doc = "Returns a future that sends the request and returns the parsed response body."]
            #[doc = ""]
            #[doc = "You should not normally call this method directly, simply invoke `.await` which implicitly calls `IntoFuture::into_future`."]
            #[doc = ""]
